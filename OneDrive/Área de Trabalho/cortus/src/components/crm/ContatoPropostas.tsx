...
interface ContatoPropostasProps {
  contato: Contato
}
...
export default function ContatoPropostas({ contato }: ContatoPropostasProps) {
  ...
  const negociosIds = negociosDoContato.map(n => n.id)
  
  // Filtrar propostas dos negócios deste contato
  const propostasDoContato = propostas.filter(p => negociosIds.includes(p.negocio_id))
  ...
  return (
    <div className="space-y-4">
      <h3 className="text-lg font-semibold text-gray-900 dark:text-white/90">
        Propostas ({propostasDoContato.length})
      </h3>

      {propostasDoContato.length === 0 ? (
        <div className="text-center py-12 px-4">
          <div className="max-w-md mx-auto">
            ...
            <p className="text-sm text-gray-500 dark:text-gray-400">
              As propostas criadas para os negócios deste contato aparecerão aqui.
            </p>
          </div>
        </div>
      ) : (
        <div className="space-y-3">
          {propostasDoContato.map((proposta) => (
            <div
              key={proposta.id}
            ...
            >
              <div className="flex items-start justify-between">
                ...
                {proposta.negocio_id && (
                  <Link href={`/crm/negocios/${proposta.negocio_id}`}>
                    <Button
                      variant="outline"
                      size="sm"
